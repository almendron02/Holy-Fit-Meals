...
export default function Home() {
  ...
  return (
    <main className="pt-16 md:pt-24">
      <section className="relative px-4 md:px-6 py-12 md:py-24 overflow-hidden">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
          ...
          >
            <span className="inline-block bg-secondary-container text-on-secondary-container font-bold px-3 py-1 md:px-4 md:py-1.5 rounded-full mb-4 md:mb-6 text-xs md:text-sm uppercase tracking-wider">FUEL YOUR POTENTIAL</span>
            <h1 className="text-4xl md:text-7xl font-headline font-extrabold text-on-surface leading-tight mb-4 md:mb-6 tracking-tight">
              Freshly Prepared,<br /><span className="text-primary italic">Miami Inspired.</span>
            </h1>
            <p className="text-base md:text-xl text-on-surface-variant leading-relaxed mb-6 md:mb-8 max-w-lg mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Link to="/menu">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="hero-gradient px-8 py-4 md:px-10 md:py-5 rounded-full text-on-primary font-headline font-extrabold text-base md:text-lg shadow-ambient flex items-center justify-center gap-2"
                >
                  SEE MENU
          ...
          >
            <div className="aspect-square max-w-[400px] mx-auto lg:max-w-none rounded-lg overflow-hidden shadow-2xl relative rotate-3 hover:rotate-0 transition-transform duration-500">
              <img
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="bg-surface-container rounded-lg p-1 text-center overflow-hidden">
              <div className="p-6 md:p-8">
                <h4 className="text-xl md:text-2xl font-headline font-bold mb-2 md:mb-4">Miami Delivery Zones</h4>
                <p className="text-xs md:text-sm text-on-surface-variant mb-4 md:mb-6 italic">We currently serve Miami-Dade and parts of Broward County.</p>
              </div>
              <div className="bg-surface-container-highest h-64 md:h-96 w-full rounded-lg relative overflow-hidden grayscale contrast-125">
      ...
      {/* Pricing Section */}
      <section className="bg-surface-container-low py-12 md:py-20 px-4 md:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10 md:mb-16">
            <h2 className="text-3xl md:text-4xl font-headline font-extrabold text-on-surface mb-2 md:mb-4">Choose Your Fuel Level</h2>
            <p className="text-sm md:text-base text-on-surface-variant">Flexible plans for every lifestyle. No commitments, skip or cancel anytime.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            {[
              ...
              <motion.div
                key={idx}
                whileHover={{ y: -10 }}
                className={`${plan.popular ? 'hero-gradient text-on-primary' : 'bg-surface-container-lowest'} rounded-lg p-6 md:p-8 flex flex-col h-full shadow-xl relative overflow-hidden`}
              >
                {plan.popular && <div className="absolute top-0 right-0 bg-tertiary-fixed text-on-tertiary-fixed px-4 py-1 font-bold text-[10px] md:text-xs uppercase tracking-widest rounded-bl-lg">Most Popular</div>}
                <div className="mb-4 md:mb-6">
                  <h3 className="text-xl md:text-2xl font-headline font-bold mb-1 md:mb-2">{plan.name}</h3>
                  <p className={`${plan.popular ? 'text-on-primary/80' : 'text-on-surface-variant'} text-xs md:text-sm`}>{plan.desc}</p>
                </div>
                <div className="mb-6 md:mb-8">
                  <span className="text-3xl md:text-4xl font-headline font-extrabold">${plan.price}</span>
                ...
                <button 
                  ...
                  className={`w-full py-3 md:py-4 rounded-xl font-bold transition-all ${plan.popular ? 'bg-white text-primary' : 'border-2 border-primary text-primary hover:bg-primary hover:text-white'} disabled:opacity-50 text-sm md:text-base`}
                >
